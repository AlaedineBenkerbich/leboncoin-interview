...
/**
 * Unit tests of [GetAlbumsInteractor].
 */
class GetAlbumsInteractorTest {

    private val mockPicturesRepository: PicturesRepository = mockk()
    private val interactor: GetAlbumsInteractor by lazy {
        GetAlbumsInteractor(mockPicturesRepository)
    }
    ...
    fun `should emit Progress result when repository emits Progress`() = runTest {
        ...
        // Then
        assertEquals(
            listOf<GetAlbumsInteractor.Result>(GetAlbumsInteractor.Result.Progress),
            result
        )
    }
    ...
    fun `should emit Error result when repository emits Error`() = runTest {
        ...
        // Then
        assertEquals(
            listOf<GetAlbumsInteractor.Result>(GetAlbumsInteractor.Result.Error),
            result
        )
    }

    @Test
    fun `should emit Success result with albums when repository emits Success with pictures`() =
        runTest {
            // Given
            val pictures = listOf(
                PictureData(
            ...
            // Then
            assertEquals(
                listOf<GetAlbumsInteractor.Result>(
    ...
    @Test
    fun `should emit Success result without albums when repository emits Success without pictures`() =
        runTest {
            // Given
            every { mockPicturesRepository.getPictures() } returns flowOf(
                PicturesRepository.FetchResult.Success(
