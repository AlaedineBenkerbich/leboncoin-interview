...
/**
 * Unit tests of [GetPictureDetailsInteractor].
 */
class GetPictureDetailsInteractorTest {

    private val mockPicturesRepository: PicturesRepository = mockk()
    private val interactor: GetPictureDetailsInteractor by lazy {
        GetPictureDetailsInteractor(mockPicturesRepository)
    }
    ...
    fun `should invoke repository when invoking interactor`() = runTest {
        ...
        // When
        interactor(pictureId = 52).collect()
    ...
    fun `should emit Progress result when repository emits Progress`() = runTest {
        ...
        // When
        val result = interactor(pictureId = 52).toList()

        // Then
        assertEquals(
            listOf<GetPictureDetailsInteractor.Result>(GetPictureDetailsInteractor.Result.Progress),
            result
        )
    }
    ...
    fun `should emit Error result when repository emits Error`() = runTest {
        ...
        // Then
        assertEquals(
            listOf<GetPictureDetailsInteractor.Result>(GetPictureDetailsInteractor.Result.Error),
            result
        )
    }

    @Test
    fun `should emit Success result with picture details when repository emits Success with pictures and picture exists`() =
        runTest {
            // Given
            val pictures = listOf(
                PictureData(
            ...
            // Then
            assertEquals(
                listOf<GetPictureDetailsInteractor.Result>(
    ...
    @Test
    fun `should emit Error result when repository emits Success with pictures and picture doesn't exist`() =
        runTest {
            // Given
            val pictures = listOf(
                PictureData(
            ...
            // When
            val result = interactor(pictureId = 200).toList()
